...
def is_useless_rule(line: str) -> bool:
    ...
    invalid_patterns = [
        ...
        # 参数类规则
        r'\$removeparam=', r'\$redirect=', r'\$rewrite=', r'\$domain=', r'\$third-party',

        # JS/CSS/图片路径类
        r'/[a-zA-Z0-9_\-]+(\.js|\.css|\.png|\.jpg|\.gif|\.svg|\.json)',

        # 追踪事件参数类
        r'%2Frw%2Fbeacon_', r'EventType=Impression', r'EventType=DataDealImpression',
        r'event_name=', r'pageview', r'gtm=', r'ga_blocked', r'popunder', r'zoneid=',
        r'&ad=', r'\badid\b', r'\bcbiframe\b', r'\bping\b', r'&referer=',

        # 通配符过多（会让 AdGuard 失效）
        r'\*{2,}', r'^\*', r'\*\.', r'\.\*'
    ]
    return any(re.search(p, line, re.IGNORECASE) for p in invalid_patterns)
